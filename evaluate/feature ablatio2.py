...
# 特征维度配置
FLOW_DIM = 60  # 流特征维度 (0-59)
SEQ_DIM = 32  # 序列特征维度
SEQ_START_IDX = 60  # 序列特征起始全局索引
SEQ_END_IDX = 91  # 序列特征结束全局索引
NUM_CLASSES = 5
...
RESULT_DIR = "ablation_results_seq_all"  # 全部序列特征消融结果目录
...
# 消融实验配置 - 一次性消融全部32维序列特征
ABLATION_FEATURES = list(range(SEQ_START_IDX, SEQ_END_IDX + 1))  # 60-91号序列特征
...
def ablation_all_seq_features(data, feature_indices, ablation_value=ABLATION_VALUE, is_test_data=True):
    """
    一次性消融多个指定全局索引的序列特征（置零）
    feature_indices: 要消融的特征索引列表，如[60,61,...,91]
    """
    # 验证所有索引是否在有效范围内
    for idx in feature_indices:
        if idx < SEQ_START_IDX or idx > SEQ_END_IDX:
            raise ValueError(f"序列特征全局索引超出范围（{SEQ_START_IDX}-{SEQ_END_IDX}）: {idx}")

    # 转换为局部索引列表（0-31）
    seq_local_indices = [idx - SEQ_START_IDX for idx in feature_indices]

    if is_test_data:
        ablated_data = data.copy()
        ablated_seq = ablated_data["features_seq"].copy()
        # 一次性消融所有指定特征
        for local_idx in seq_local_indices:
            ablated_seq[:, local_idx] = ablation_value
        ablated_data["features_seq"] = ablated_seq
    else:
        ablated_data = {cls: [] for cls in CLASSES}
        for cls in CLASSES:
            for flow, seq in data[cls]:
                ablated_seq = seq.copy()
                # 一次性消融所有指定特征
                for local_idx in seq_local_indices:
                    ablated_seq[local_idx] = ablation_value
                ablated_data[cls].append((flow, ablated_seq))
...
# 2. 混淆矩阵计算工具（支持不同评估场景）
# =====================================================
def compute_confusion_matrix(model, data_source, sampler=None, test_data=None, feature_desc="all", eval_type="val"):
    """
    计算混淆矩阵（支持验证集/测试集）
    eval_type: "val" (训练集验证) / "test" (独立测试集)
    feature_desc: 特征描述，用于文件名
    """
    model.eval()
    all_true = []
    all_pred = []
    ...
    if CONFUSION_MATRIX_SAVE:
        ...
        cm_filename = f"confusion_matrix_seq_{feature_desc}_{eval_type}.csv"
        ...
        plt.title(f'混淆矩阵（全部序列特征消融 - {eval_name}）')
        plt.colorbar()
        ...
        plt.xlabel('预测类别')
        plt.tight_layout()
        fig_filename = f"confusion_matrix_seq_{feature_desc}_{eval_type}.png"
...
class AblationTrainer:
    ...
    def train(self, sampler, feature_desc):
        """训练消融模型，返回最佳验证准确率"""
        print(f"\n🚀 开始训练消融全部序列特征后的模型...")
        ...
        for epoch in range(META_EPOCHS):
            ...
            if (epoch + 1) % 5 == 0:
                ...
                if val_acc > self.best_val_acc:
                    ...
                    # 保存最佳验证模型
                    model_path = os.path.join(RESULT_DIR, f"ablated_model_seq_all.pth")
                    torch.save(self.model.state_dict(), model_path)

        print(f"✅ 消融全部序列特征模型训练完成 | 最佳验证准确率: {self.best_val_acc:.4f} (Epoch {self.best_val_epoch})")
...
# 5. 双评估核心函数
# =====================================================
def evaluate_ablation_model(model, feature_desc, ablated_train_data, ablated_test_data, gan_data):
    ...
    val_cm, _, val_acc = compute_confusion_matrix(
        model, None, sampler=val_sampler, feature_desc=feature_desc, eval_type="val"
    )

    # 3. 评估2：测试集最终准确率
    test_cm, _, test_acc = compute_confusion_matrix(
        model, None, test_data=ablated_test_data, feature_desc=feature_desc, eval_type="test"
    )
    ...
    if CONFUSION_MATRIX_SAVE:
        cm_df = pd.DataFrame(final_cm,
                             index=[f'真实_{cls}' for cls in CLASSES],
                             columns=[f'预测_{cls}' for cls in CLASSES])
        cm_path = os.path.join(RESULT_DIR, f"confusion_matrix_seq_all_final.csv")
        cm_df.to_csv(cm_path, encoding='utf-8')
        ...
        plt.title(f'混淆矩阵（全部序列特征消融 - 最终指标/{final_type}）')
        plt.colorbar()
        tick_marks = np.arange(len(CLASSES))
        plt.xticks(tick_marks, CLASSES, rotation=45)
        plt.yticks(tick_marks, CLASSES)
        ...
        plt.xlabel('预测类别')
        plt.tight_layout()
        plt.savefig(os.path.join(RESULT_DIR, f"confusion_matrix_seq_all_final.png"), dpi=300)
        plt.close()
...
def evaluate_baseline_model(model, test_data):
    ...
    # 计算测试集混淆矩阵和准确率
    test_cm, _, test_acc = compute_confusion_matrix(
        model, None, test_data=test_data, feature_desc="baseline", eval_type="test"
    )
...
def main():
    ...
    # Step 3: 一次性消融全部序列特征
    print("\n" + "=" * 80)
    print("Step 2: 一次性消融全部序列特征")
    print("=" * 80)

    print(f"\n🔬 开始消融全部序列特征 {ABLATION_FEATURES[0]}-{ABLATION_FEATURES[-1]}...")

    # a. 一次性消融全部序列特征
    ablated_train_data = ablation_all_seq_features(train_data, ABLATION_FEATURES, is_test_data=False)
    ablated_test_data = ablation_all_seq_features(test_data, ABLATION_FEATURES, is_test_data=True)
    ...
    best_val_acc = ablation_trainer.train(ablated_sampler, "all")

    # c. 双评估+取低值
    eval_results = evaluate_ablation_model(
        ablated_model, "all", ablated_train_data, ablated_test_data, gan_data
    )
    ...
    # e. 保存结果
    ablation_results = {
        "feature_desc": "全部序列特征(60-91)",
        "val_acc": eval_results["val_acc"],
        "test_acc": eval_results["test_acc"],
        "final_acc": eval_results["final_acc"],
        "final_type": eval_results["final_type"],
        "acc_drop": acc_drop,
        "baseline_acc": baseline_acc
    }

    with open(os.path.join(RESULT_DIR, "ablation_all_seq_results.pkl"), 'wb') as f:
        pickle.dump(ablation_results, f)

    # 转换为DataFrame并保存CSV
    results_df = pd.DataFrame([ablation_results])
    results_df.to_csv(os.path.join(RESULT_DIR, "ablation_all_seq_results.csv"), index=False, encoding='utf-8')

    print(f"\n📝 消融全部序列特征最终结果:")
    print(f"   最终取: {eval_results['final_type']} ({eval_results['final_acc']:.4f})")
    print(f"   基准准确率: {baseline_acc:.4f}")
    print(f"   相对于基准下降: {acc_drop:.4f}")

    # Step 4: 可视化对比
    print("\n" + "=" * 80)
    print("Step 3: 实验结果可视化对比")
    print("=" * 80)
    ...
    if SAVE_PLOTS:
        ...
        categories = ['基准模型', '全部序列特征消融']
        values = [baseline_acc, eval_results['final_acc']]
        ...
        # 添加下降幅度标注
        plt.text(0.5, (baseline_acc + eval_results['final_acc']) / 2,
                 f'下降幅度: {acc_drop:.4f}',
                 ha='center', va='center', fontsize=12, color='red',
                 bbox=dict(boxstyle='round,pad=0.3', facecolor='yellow', alpha=0.3))

        plt.ylabel('准确率', fontsize=13)
        plt.title('一次性消融全部序列特征(60-91)性能对比', fontsize=14, fontweight='bold')
        plt.ylim(0, 1.0)
        plt.grid(axis='y', linestyle='--', alpha=0.3)

        plt.tight_layout()
        plt.savefig(os.path.join(RESULT_DIR, "ablation_all_seq_comparison.png"), dpi=300, bbox_inches='tight')
        plt.close()
    ...
    print(f"\n🔍 一次性消融全部序列特征实验结果:")
    print(f"   消融特征范围: 60-91（共{len(ABLATION_FEATURES)}维）")
    print(f"   消融模型最终准确率: {eval_results['final_acc']:.4f}")
    print(f"   下降幅度: {acc_drop:.4f}")
    print(f"   性能保留率: {(eval_results['final_acc'] / baseline_acc) * 100:.2f}%")

    print(f"\n💡 关键结论:")
    if acc_drop > 0.1:
        print(f"   全部序列特征对模型性能有显著影响，消融后准确率下降{acc_drop:.4f}")
    elif acc_drop > 0.05:
        print(f"   全部序列特征对模型性能有中等程度影响，消融后准确率下降{acc_drop:.4f}")
    else:
        print(f"   全部序列特征对模型性能影响较小，消融后准确率仅下降{acc_drop:.4f}")
